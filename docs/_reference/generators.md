---
title: Generators
description: The two real OpenLoam generators — open_loam:install and open_loam:entity — with their actual options.
nav_order: 2
...
1. **Migrations** for every OpenLoam-owned table (tenants, memberships, audit
...
2. **ActiveStorage install** (attachments need it — generated entities
   `include OpenLoam::Attachable`; skipped with a warning if ActiveStorage isn't
   available).
3. **A minimal `User` model** with `has_secure_password`.
4. **`config/initializers/open_loam.rb`** — see [Configuration]({% link _reference/configuration.md %}).
...
8. **`test/open_loam_guardrails_test.rb`** — the structural guardrail suite. See
   [Guardrails]({% link _agents/guardrails.md %}).
9. **Test-helper wiring** — if `test/test_helper.rb` exists, injects
   `require "open_loam/test_helpers"` and `include OpenLoam::TestHelpers`. If the app
...
| `--encrypt field field` | `[]` | Encrypt these fields at rest, per tenant (`OpenLoam::Encryptable`). |
...
- `test/entities/<name>_test.rb` — the generated guardrail/isolation tests
  ...
  `lib/generators/open_loam/entity/templates/entity_test.rb`.
