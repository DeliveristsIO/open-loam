...
If you are an AI agent running under confirm-mode (`OpenLoam.require_confirmation?`
...
This records a `OpenLoam::PendingAction` with a before/after preview and touches
...
OpenLoam does not auto-gate Active Record, so a direct `update!` still writes
