...
Recurring jobs, per tenant. `OpenLoam::Scheduler.register(key:, job_class:, schedule:,
...
`job_class` MUST be a real ActiveJob (validated). `bin/rails open_loam:scheduler:tick`
