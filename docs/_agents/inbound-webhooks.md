...
system POSTs to `/webhooks/:token`; Loam verifies it, dedupes replays, records it,
and publishes it on the domain event bus so durable subscribers react.

## Setup (admin, manager-only)

`/admin/inbound_webhook_sources` → **New source**. Loam generates:
...
| `signature_header` | where the signature arrives (default `X-Loam-Signature`; GitHub uses `X-Hub-Signature-256`) |
...
## The pipeline (`Loam::InboundWebhooks.ingest`)
...
The verified body is stored on a `Loam::InboundWebhookDelivery` row; the published
...
```ruby
Loam::DurableEvents.register(key: "on_inbound", to: "billing.invoice.paid",
                             call: "Billing::ApplyPaymentHandler")
# handler reads Loam::InboundWebhookDelivery.find(payload["delivery_id"]).payload_hash
```
...
- **Tenant isolation:** the token establishes the tenant (a blessed cross-tenant
  lookup, like `Loam::ApiToken.authenticate`), and the controller resets
  `Loam::Current` after every request.

## Notes / follow-ups

- The secret is stored plaintext, matching the outbound `Loam::WebhookEndpoint`
...
- Delivery rows accumulate — add a periodic prune for a high-volume source
  (a `loam:inbound:prune` task is the obvious follow-up).
