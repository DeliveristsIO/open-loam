...
`loam:install` writes an `AGENTS.md` to every generated app's root
(from [the shipped template](https://github.com/DeliveristsIO/open-loam/blob/main/lib/generators/loam/install/templates/AGENTS.md)).
...
> This app is built on Loam: a Rails foundation where tenancy, permissions,
...
1. **The map.** A table of every feature (business entity, permissions,
   ...
   scheduler, dictionaries, and everything else Loam ships) — where it lives
   and how to add one. Reading this table before writing code is how an
   agent avoids reinventing something Loam already provides.
...
3. **Invariants you must not break** — the load-bearing rules: every business
   model inherits `Loam::TenantRecord`; never `.unscoped` a tenant-scoped
   model; never rescue `Loam::MissingTenantError` or
   `Loam::UnknownCustomFieldError`; every controller action checks a policy;
   event names are `domain.thing.happened`; `Loam.on_tenant_created`
