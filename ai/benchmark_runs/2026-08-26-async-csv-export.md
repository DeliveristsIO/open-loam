# Async CSV export — Vanilla Rails vs OpenLoam

An internal, single-task A/B run comparing an established conventional Rails
baseline with an equivalent Rails + OpenLoam baseline. Both coding-agent sessions
...
This run measures whether OpenLoam changes the implementation surface. It does
**not** establish a correctness advantage: both implementations passed every
hidden contract assertion.

## Headline

| | Vanilla Rails + AI | OpenLoam + AI |
|---|---:|---:|
...
**Correctness is a tie.** The useful signal is architectural: the OpenLoam agent
changed half as many files and reused tenant context, authorization,
`OpenLoam::Export`, and `OpenLoam::ProgressJob`. The Vanilla agent had to introduce its
...
less typing. Both agents wrote extensive tests, and the OpenLoam agent added 77
lines of ADR/lessons documentation. The narrower OpenLoam result is about fewer
...
The OpenLoam implementation added an Active Job and customer-controller/view
actions, but stored export status and output in the existing tenant-scoped
`OpenLoam::ProgressJob`. It generated content with `OpenLoam::Export.csv`, carried
...
- OpenLoam's entity generator already supplied a synchronous, manager-gated,
  policy-aware CSV export. The OpenLoam task extended existing export machinery to
...
- The harness stored a mutable absolute path to the OpenLoam checkout rather than
  its commit SHA. The exact OpenLoam source revision used by the run cannot be
...
- Both implementations persist the complete CSV in a database column. That is
  ...
  large exports.
- The OpenLoam `CustomerExportJob` catches generation errors, marks its
  `OpenLoam::ProgressJob` failed, and does not re-raise. The queue therefore sees a
  successful execution and will not apply its normal retry policy. The Vanilla
  job records failure and re-raises. A production OpenLoam implementation should
...
> Both agents produced correct and tenant-safe features. In this run, OpenLoam did
> not improve the pass rate; it reduced the amount of application-specific
> architecture needed to reach the same result.

That distinction matters. This run supports OpenLoam's composability and reduced
plumbing thesis. It should not be cited as evidence that Vanilla Rails failed,
that OpenLoam was faster, or that OpenLoam improved correctness in this task.
...
- [OpenLoam complete baseline-to-feature patch](artifacts/2026-08-26-async-csv-export/loam-full.diff)
