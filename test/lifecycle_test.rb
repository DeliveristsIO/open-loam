...
# registered with Loam.on_tenant_created runs when a tenant is created, runs
# inside that tenant's context, and — because `bin/rails loam:sync` re-runs it
...
class LifecycleTest < HarnessCase
  ...
  # app would write one. find_or_create_by! is the contract Loam::Lifecycle
  ...
  SEED_HOOK = <<~RUBY.freeze
    ...
    # Appended by the Loam harness.
    Loam.on_tenant_created do |tenant|
      Loam::FieldDefinition.find_or_create_by!(entity_type: "Gadget", name: "asset_tag") do |definition|
  ...
  def test_on_tenant_created_fires_on_create_and_loam_sync_backfills_existing_tenants
    app = build_app(name: "loam_lifecycle_app")

    step("rails g loam:install", "bin/rails g loam:install", app)
    step("rails db:migrate", "bin/rails db:migrate", app)
    step("rails g loam:entity Gadget", "bin/rails g loam:entity Gadget name:string --domain lab", app)
    ...
    File.write(File.join(app, "config/initializers/loam.rb"), SEED_HOOK, mode: "a")

    # (1) The hook fires on create...
    create_tenant("new", app)
    assert_equal 1, seeded_fields("new", app),
                 "Loam.on_tenant_created did not run when the tenant was created"
    ...
    # can only have come from Loam.as_tenant.
    ...
    sync = step("rails loam:sync", "bin/rails loam:sync", app)
    assert_match(/tenant/i, sync.output,
                 "loam:sync produced no report of what it did#{sync.failure_report}")

    assert_equal 1, seeded_fields("old", app),
                 "bin/rails loam:sync did not backfill the tenant that predated the hook#{sync.failure_report}"
    assert_equal 1, seeded_fields("new", app),
                 "bin/rails loam:sync duplicated the seed in a tenant that already had it#{sync.failure_report}"
    ...
    resync = step("rails loam:sync (again)", "bin/rails loam:sync", app)
    assert_equal 1, seeded_fields("old", app), "a second loam:sync duplicated the seed#{resync.failure_report}"
    assert_equal 1, seeded_fields("new", app), "a second loam:sync duplicated the seed#{resync.failure_report}"
  end

  private

  def create_tenant(slug, app)
    runner("create tenant #{slug}", %(Loam::Tenant.create!(name: "Tenant #{slug}", slug: "#{slug}")), app)
  end
  ...
  def seeded_fields(slug, app)
    runner_integer("count seeds in #{slug}", in_tenant(slug, "Loam::FieldDefinition.count"), app)
  end

  def tenant_id(slug, app)
    runner_integer("tenant id #{slug}", %(Loam::Tenant.find_by!(slug: "#{slug}").id), app)
  end

  def field_tenant_id(slug, app)
    runner_integer("seed tenant_id in #{slug}", in_tenant(slug, "Loam::FieldDefinition.first.tenant_id"), app)
  end
  ...
  def in_tenant(slug, expression)
    %(Loam.as_tenant(Loam::Tenant.find_by!(slug: "#{slug}")) { #{expression} })
  end
end
