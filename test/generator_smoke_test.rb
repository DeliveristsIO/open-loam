...
# The end-to-end proof: a virgin Rails app, the two generators that are Loam's
...
class GeneratorSmokeTest < HarnessCase
  # Files loam:install must put in the host app. Their contents are the
  ...
  ENTITY_COMMAND = "bin/rails g loam:entity Gadget name:string price:decimal --domain lab"

  def test_loam_installs_into_a_brand_new_rails_app_and_leaves_it_green
    app = build_app

    # (a) install
    install = step("rails g loam:install", "bin/rails g loam:install", app, timeout: 180)

    missing = INSTALL_ARTIFACTS.reject { |f| File.file?(File.join(app, f)) }
    assert_empty missing, "loam:install did not create: #{missing.join(', ')}#{install.failure_report}"
    ...
    # The install wires Loam's test helpers into the host app's test_helper.rb
    # by injection, which is the step most likely to silently no-op when Rails
    # changes its generated test_helper.
    assert_includes File.read(File.join(app, "test/test_helper.rb")), "Loam::TestHelpers",
                    "loam:install did not wire Loam::TestHelpers into test/test_helper.rb#{install.failure_report}"

    # (b) migrate
    step("rails db:migrate (install)", "bin/rails db:migrate", app, timeout: 180)

    # (c) entity
    entity = step("rails g loam:entity Gadget", ENTITY_COMMAND, app, timeout: 180)

    missing = ENTITY_ARTIFACTS.reject { |f| File.file?(File.join(app, f)) }
    assert_empty missing, "loam:entity did not create: #{missing.join(', ')}#{entity.failure_report}"
    ...
    assert_match(/resources :gadgets/, File.read(File.join(app, "config/routes.rb")),
                 "loam:entity did not add the admin route for Gadget#{entity.failure_report}")
    ...
    rerun = attempt("rails g loam:entity Gadget (rerun)", "#{ENTITY_COMMAND} --skip", app)
    refute rerun.timed_out?, "the second loam:entity run hung — it is prompting on a file collision#{rerun.failure_report}"

    duplicates = Dir[File.join(app, "db/migrate/*_create_gadgets.rb")]
    assert_equal 1, duplicates.size,
                 "re-running loam:entity added a duplicate migration: #{duplicates.map { |f| File.basename(f) }.join(', ')}" \
