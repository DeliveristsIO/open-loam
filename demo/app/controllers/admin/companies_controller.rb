module Admin
  class CompaniesController < BaseController
    ...
    def index
      @perspective = Loam::Perspectives.resolve("Company", user: current_actor, id: params[:perspective_id])
      @perspectives = Loam::Perspectives.visible_to("Company", user: current_actor)
    ...
    def bulk
      ...
      when "soft_delete"
        count = Loam::Bulk.soft_delete(Company, ids)
        redirect_to polymorphic_path([:admin, Company]), notice: "Deleted #{count} record(s)."
      when "set_field"
        count = Loam::Bulk.set_field(Company, ids, field: params[:field], value: params[:value])
      ...
      when "export"
        ...
        send_data Loam::Export.csv(Loam::Bulk.selected(Company, ids), actor: current_actor),
    ...
    def create
      ...
      if @record.save
        redirect_to [:admin, @record], notice: t("loam.flash.created", name: Company.model_name.human)
      else
    ...
    def update
      ...
      if @record.update(permitted_params(policy))
        ...
        redirect_to [:admin, @record], notice: t("loam.flash.updated", name: Company.model_name.human)
      else
    ...
    def destroy
      ...
      @record.soft_delete!
      Loam::RecordLocks.release(@record, by: current_actor)
      redirect_to [:admin, Company], notice: t("loam.flash.destroyed", name: Company.model_name.human)
    end
    ...
    def restore
      ...
      authorize!(policy_for(@record), :update?)
      @record.restore!
      redirect_to [:deleted, :admin, Company], notice: t("loam.flash.restored", name: Company.model_name.human)
    end
    ...
    def apply_custom_field_filter(scope)
      ...
      scope.merge(Loam::CustomFieldIndex.filter(Company, params[:cf_field], params[:cf_op].presence || "eq", params[:cf_value]))
    rescue Loam::Error
      scope
    end
