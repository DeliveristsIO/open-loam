module Admin
  class CompaniesController < BaseController
    FIELDS = %i[name industry tier].freeze
    ...
    def index
      @perspective = OpenLoam::Perspectives.resolve("Company", user: current_actor, id: params[:perspective_id])
      @perspectives = OpenLoam::Perspectives.visible_to("Company", user: current_actor)
    ...
    def export
      ...
      send_data OpenLoam::Export.csv(index_scope, actor: current_actor),
                filename: "companies-#{Date.current}.csv", type: "text/csv"
    end
    ...
    def bulk
      ...
      when "soft_delete"
        count = OpenLoam::Bulk.soft_delete(Company, ids)
        redirect_to polymorphic_path([:admin, Company]), notice: "Deleted #{count} record(s)."
      when "set_field"
        count = OpenLoam::Bulk.set_field(Company, ids, field: params[:field], value: params[:value])
        redirect_to polymorphic_path([:admin, Company]), notice: "Updated #{count} record(s)."
      when "export"
        require_role!(:manager)  # same gate as the dedicated export action
        send_data OpenLoam::Export.csv(OpenLoam::Bulk.selected(Company, ids), actor: current_actor),
                  filename: "companies-selected.csv", type: "text/csv"
      else
        redirect_to polymorphic_path([:admin, Company]), alert: "Unknown bulk action."
      end
    end
    ...
    def deleted
      authorize!(policy_for(Company.new), :read?)
      scope = Company.only_deleted.order(deleted_at: :desc, id: :desc)
    ...
    def create
      ...
      if @record.save
        redirect_to [:admin, @record], notice: t("open_loam.flash.created", name: Company.model_name.human)
      else
    ...
    def update
      ...
      if @record.update(permitted_params(policy))
        ...
        redirect_to [:admin, @record], notice: t("open_loam.flash.updated", name: Company.model_name.human)
      else
    ...
    def destroy
      ...
      @record.soft_delete!
      OpenLoam::RecordLocks.release(@record, by: current_actor)
      redirect_to [:admin, Company], notice: t("open_loam.flash.destroyed", name: Company.model_name.human)
    end
    ...
    def restore
      @record = Company.with_deleted.find(params[:id])
      authorize!(policy_for(@record), :update?)
      @record.restore!
      redirect_to [:deleted, :admin, Company], notice: t("open_loam.flash.restored", name: Company.model_name.human)
    end
    ...
    def apply_custom_field_filter(scope)
      ...
      scope.merge(OpenLoam::CustomFieldIndex.filter(Company, params[:cf_field], params[:cf_op].presence || "eq", params[:cf_value]))
    ...
    def permitted_params(policy)
      params.require(:company).permit(*policy.permitted_fields(FIELDS), :lock_version)
    end
  end
end
