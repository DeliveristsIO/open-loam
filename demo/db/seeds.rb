...
warsaw = OpenLoam::Tenant.find_or_create_by!(slug: "warsaw") { |t| t.name = "Branch Warsaw" }
krakow = OpenLoam::Tenant.find_or_create_by!(slug: "krakow") { |t| t.name = "Branch Krakow" }

# Per-tenant defaults (the asset_tag field definition) come from the
# OpenLoam.on_tenant_created hook in config/initializers/open_loam.rb, which fires only
# on creation. Syncing backfills tenants that already existed — the same thing
# `bin/rails open_loam:sync` does after a deploy that adds a new default.
OpenLoam.sync_tenants!

# Settings (OpenLoam::Configs). The declared default for the late fee is 25 (see
# config/initializers/open_loam.rb); this global row raises the company-wide baseline
...
OpenLoam::Configs.set("rental.late_fee_per_day", 30, scope: :global)
...
OpenLoam.as_tenant(warsaw, actor: anna) do
  OpenLoam::Membership.find_or_create_by!(user: anna) { |m| m.role = "manager" }
  OpenLoam::Membership.find_or_create_by!(user: tomek) { |m| m.role = "employee" }

  # Warsaw charges a higher late fee than the company baseline.
  OpenLoam::Configs.set("rental.late_fee_per_day", 45)
  ...
  OpenLoam::Features.enable(:beta_dashboard)

  # Migration-free fields: added from the admin, not a generator.
  OpenLoam::FieldDefinition.find_or_create_by!(entity_type: "Equipment", name: "serial_number") do |fd|
    fd.field_type = "string"
  end
  OpenLoam::FieldDefinition.find_or_create_by!(entity_type: "Equipment", name: "warranty_expires_at") do |fd|
    fd.field_type = "date"
  ...
  # Content translations (OpenLoam::Translatable): the excavator's NAME per locale.
  ...
  # A customer with encrypted PII at rest (OpenLoam::Encryptable). email is
  ...
  # Saved views of the Equipment list (OpenLoam::Perspectives): a tenant-wide default
  # that shows only available gear, and a private view Anna sorts by price.
  OpenLoam::Perspective.find_or_create_by!(entity_type: "Equipment", name: "Available only", visibility: "tenant") do |p|
    p.is_default = true
    p.config = { "filters" => { "status" => "available" }, "sort" => { "field" => "name", "dir" => "asc" } }
  end
  OpenLoam::Perspective.find_or_create_by!(entity_type: "Equipment", name: "Anna's priciest", owner_id: anna.id, visibility: "private") do |p|
    p.config = { "sort" => { "field" => "daily_rate", "dir" => "desc" } }
  end

  # A business rule (OpenLoam::BusinessRules): WHEN a damage report is submitted AND
  ...
  OpenLoam::BusinessRule.find_or_create_by!(name: "Flag urgent damage reports") do |rule|
  ...
  # An AI-proposed price change waiting in the approval queue (OpenLoam::PendingActions):
  ...
  # An SSO provider (OpenLoam::SsoProvider): anyone with a warsaw-corp.example email
  ...
  OpenLoam::SsoProvider.find_or_create_by!(domain: "warsaw-corp.example") do |p|
    p.name = "Warsaw Corp SSO"
    p.protocol = "oidc"
    p.issuer = "https://idp.warsaw-corp.example"
    p.client_id = "open_loam-demo"
  ...
  OpenLoam::FieldDefinition.find_or_create_by!(entity_type: "DamageReport", name: "severity") do |fd|
    fd.field_type = "dictionary"
    fd.dictionary_key = "damage_severity"
  end

  # A configured dashboard (OpenLoam::DashboardWidget): audit + pending approvals +
  ...
  [ %w[audit_recent 1], %w[pending_approvals 2], %w[open_progress 3] ].each do |key, pos|
    OpenLoam::DashboardWidget.find_or_create_by!(widget_key: key) { |dw| dw.position = pos.to_i }
  end

  # A finished long-running task (OpenLoam::ProgressJob) so the Tasks screen shows
  # history; run a live one from that screen's "Run a demo job" button.
  OpenLoam::ProgressJob.find_or_create_by!(name: "Nightly reindex") do |job|
    job.total = 42
    job.completed = 42
  ...
  # The demo uses the TokenDriver (see config/initializers/open_loam.rb). New records
  ...
  # the same thing `bin/rails open_loam:search:reindex` does after a deploy.
  [ Equipment, Customer, DamageReport ].each do |model|
    OpenLoam::Search.reindex(model)
    OpenLoam::CustomFieldIndex.reindex(model) # read-model index for custom-field filter/sort
  end
end

OpenLoam.as_tenant(krakow, actor: anna) do
  OpenLoam::Membership.find_or_create_by!(user: anna) { |m| m.role = "manager" }
