...
class AdminBusinessRulesTest < ActionDispatch::IntegrationTest
  setup do
    @tenant = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-rules-admin")
  ...
  test "a manager sees the rules list and the recent-runs table" do
    with_tenant(@tenant) do
      rule = OpenLoam::BusinessRule.create!(name: "Flag urgent", trigger: "rental.damage_report.submit",
      ...
      OpenLoam::BusinessRuleRun.create!(business_rule: rule, event_name: "rental.damage_report.submit",
  ...
  test "a manager creates a rule with JSON condition and actions" do
    ...
    assert_difference -> { with_tenant(@tenant) { OpenLoam::BusinessRule.count } }, 1 do
    ...
    rule = with_tenant(@tenant) { OpenLoam::BusinessRule.find_by(name: "Pricey") }
  ...
  test "the edit form renders and an update persists" do
    rule = with_tenant(@tenant) do
      OpenLoam::BusinessRule.create!(name: "Old", trigger: "rental.equipment.pinged", condition: {}, actions: [])
    end
    sign_in("anna@example.test")
    ...
    assert_redirected_to admin_business_rules_path
    assert_equal "Renamed", with_tenant(@tenant) { OpenLoam::BusinessRule.find(rule.id).name }
  end

  test "malformed JSON re-renders the form as 422, never a 500" do
    sign_in("anna@example.test")

    assert_no_difference -> { with_tenant(@tenant) { OpenLoam::BusinessRule.count } } do
      post admin_business_rules_path, params: { business_rule: {
