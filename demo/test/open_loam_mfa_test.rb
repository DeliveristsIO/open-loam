...
# OpenLoam::MfaCredential + TOTP: the second factor and its per-user key. Security
...
class OpenLoamMfaTest < ActiveSupport::TestCase
  setup do
    @warsaw = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-mfa")
    @krakow = OpenLoam::Tenant.create!(name: "Branch Krakow", slug: "krakow-mfa")
  ...
  test "TOTP verification accepts a live code and rejects wrong or malformed ones" do
    ...
    assert OpenLoam::Totp.verify(secret, OpenLoam::Totp.code_at(secret, now / 30), at: now)
    assert OpenLoam::Totp.verify(secret, OpenLoam::Totp.code_at(secret, now / 30 + 1), at: now), "±1 step drift is allowed"
    refute OpenLoam::Totp.verify(secret, OpenLoam::Totp.code_at(secret, now / 30 + 2), at: now), "±2 steps is not"
    refute OpenLoam::Totp.verify(secret, "000000", at: now)
    refute OpenLoam::Totp.verify(secret, "abc", at: now)
  end
  ...
  test "the TOTP secret is encrypted at rest" do
    ...
    raw = OpenLoam::MfaCredential.connection.select_value(
      "SELECT totp_secret FROM open_loam_mfa_credentials WHERE id = #{credential.id}"
    )
  ...
  test "MFA enrolled in one tenant verifies in another tenant and with no tenant" do
    ...
    with_tenant(@krakow) do
      code = OpenLoam::Totp.code_at(secret, Time.now.to_i / 30)
      assert OpenLoam::MfaCredential.active_for(@user).verify_totp(code), "verifies in a different tenant"
    end
    ...
    travel 30.seconds do
      ...
      code = OpenLoam::Totp.code_at(secret, Time.now.to_i / 30)
      assert OpenLoam::MfaCredential.active_for(@user).verify_totp(code), "verifies with no tenant (login time)"
    end
  end

  test "a credential is not active until a live code confirms enrollment" do
    secret = OpenLoam::Totp.generate_secret
    credential = OpenLoam::MfaCredential.new(user: @user)
    assert_nil OpenLoam::MfaCredential.active_for(@user), "an unsaved/pending enrollment does not gate login"

    refute credential.activate_with!(secret, "000000"), "a wrong code does not activate"
    assert_nil OpenLoam::MfaCredential.active_for(@user)

    codes = credential.activate_with!(secret, OpenLoam::Totp.code_at(secret, Time.now.to_i / 30))
    assert_equal OpenLoam::MfaCredential::RECOVERY_CODE_COUNT, codes.size
    assert OpenLoam::MfaCredential.active_for(@user)
  end
  ...
  test "assigning a secret with no user raises rather than deriving a shared key" do
    assert_raises(OpenLoam::Encryption::Error) { OpenLoam::MfaCredential.new.totp_secret = "x" }
  end
  ...
  test "a recovery code cannot be consumed twice, even from a stale instance" do
    credential = enroll(@user)
    code = @recovery_codes.first
    stale = OpenLoam::MfaCredential.find(credential.id) # loaded BEFORE the first consume
  ...
  test "a TOTP code cannot be replayed within its validity window" do
    credential = enroll(@user)
    code = OpenLoam::Totp.code_at(credential.totp_secret, Time.now.to_i / 30)
    ...
    travel 30.seconds do
      next_code = OpenLoam::Totp.code_at(credential.totp_secret, Time.now.to_i / 30)
  ...
  def enroll(user)
    ...
    travel_to(61.seconds.ago) do
      @recovery_codes = credential.activate_with!(secret, OpenLoam::Totp.code_at(secret, Time.now.to_i / 30))
    end
    credential
  end
end
...
class AdminMfaFlowTest < ActionDispatch::IntegrationTest
  setup do
    @tenant = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-mfa-flow")
  ...
  test "a sudo-gated action re-challenges once the login is stale, then allows it" do
    post admin_session_path, params: { email: @anna.email, password: "password" }
    token = with_tenant(@tenant) { OpenLoam::ApiToken.create!(user: @anna, label: "scanner") }
    ...
    travel 6.minutes do
      other = with_tenant(@tenant) { OpenLoam::ApiToken.create!(user: @anna, label: "old") }
      ...
      assert with_tenant(@tenant) { OpenLoam::ApiToken.exists?(other.id) }, "and the token is untouched"

      post admin_sudo_path, params: { password: "password" }
      delete admin_api_token_path(other)
      assert_redirected_to admin_api_tokens_path
      refute with_tenant(@tenant) { OpenLoam::ApiToken.exists?(other.id) }, "now the delete goes through"
    end
  end
  ...
  test "step-up for an MFA user needs a TOTP code, not just a password" do
    ...
    travel 6.minutes do
      token = with_tenant(@tenant) { OpenLoam::ApiToken.create!(user: @anna, label: "old") }
      ...
      post admin_sudo_path, params: { password: "password" }
      assert_response :unauthorized
      assert with_tenant(@tenant) { OpenLoam::ApiToken.exists?(token.id) }

      # A live code does.
      post admin_sudo_path, params: { code: OpenLoam::Totp.code_at(secret, Time.now.to_i / 30) }
      delete admin_api_token_path(token)
      assert_redirected_to admin_api_tokens_path
      refute with_tenant(@tenant) { OpenLoam::ApiToken.exists?(token.id) }
    end
  end
  ...
  test "a GET to the enroll page cannot disable active MFA" do
    ...
    assert OpenLoam::MfaCredential.active_for(@anna), "MFA is still active — not downgraded to password-only"
  end
  ...
  test "replacing an active credential needs step-up and leaves it untouched until confirmed" do
    ...
    travel 6.minutes do # sudo has gone stale
      ...
      assert OpenLoam::MfaCredential.active_for(@anna)
    end
  end
  ...
  test "step-up does not accept or consume a recovery code" do
    ...
    recovery_code = nil
    travel_to(61.seconds.ago) do
      recovery_code = OpenLoam::MfaCredential.new(user: @anna).activate_with!(secret, OpenLoam::Totp.code_at(secret, Time.now.to_i / 30)).first
    end
    log_in_with_mfa(secret)

    travel 6.minutes do
      token = with_tenant(@tenant) { OpenLoam::ApiToken.create!(user: @anna, label: "x") }
      delete admin_api_token_path(token)
      assert_redirected_to new_admin_sudo_path

      post admin_sudo_path, params: { code: recovery_code }
      assert_response :unauthorized, "a recovery code must not satisfy step-up"
      assert_equal OpenLoam::MfaCredential::RECOVERY_CODE_COUNT,
                   OpenLoam::MfaCredential.active_for(@anna).unused_recovery_code_count, "and it must not be burned"
      assert OpenLoam::MfaCredential.active_for(@anna).consume_recovery_code(recovery_code), "it still works at login"
    end
  end
  ...
  def raw_secret_of(user)
    OpenLoam::MfaCredential.connection.select_value("SELECT totp_secret FROM open_loam_mfa_credentials WHERE user_id = #{user.id}")
  end
  ...
  def enroll(user)
    secret = OpenLoam::Totp.generate_secret
    travel_to(61.seconds.ago) do
      OpenLoam::MfaCredential.new(user: user).activate_with!(secret, OpenLoam::Totp.code_at(secret, Time.now.to_i / 30))
    end
    secret
  end
end
