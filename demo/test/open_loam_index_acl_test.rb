...
class LoamIndexAclTest < ActiveSupport::TestCase
  setup do
    @warsaw = Loam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-acl")
    ...
    with_tenant(@warsaw) do
      ...
      Loam::Membership.create!(user: @clerk, role: "clerk")
      Loam::FieldDefinition.create!(entity_type: "Equipment", name: "clearance",
                                    field_type: "string", readable_roles: [ "manager" ])
      Loam::FieldDefinition.create!(entity_type: "Equipment", name: "color", field_type: "string")
    end
  end

  test "readable_by? — empty roles is open, otherwise only listed roles" do
    with_tenant(@warsaw) do
      restricted = Loam::FieldDefinition.find_by!(name: "clearance")
      open_field = Loam::FieldDefinition.find_by!(name: "color")
  ...
  test "a role without read on a field cannot use it as a filter oracle" do
    with_tenant(@warsaw, actor: @clerk) do
      assert_raises(Loam::FieldAccessError) do
        Loam::CustomFieldIndex.filter(Equipment, "clearance", "eq", "top-secret")
      end
      assert_raises(Loam::FieldAccessError) do
        Loam::CustomFieldIndex.order(Equipment, "clearance", :asc)
      end
    end
  end

  test "FieldAccessError is a NotAuthorizedError (renders 403 in admin)" do
    assert Loam::FieldAccessError.ancestors.include?(Loam::NotAuthorizedError)
  end

  test "a role with read on the field filters and sorts normally" do
    with_tenant(@warsaw, actor: @manager) do
      assert_nothing_raised do
        rel = Loam::CustomFieldIndex.filter(Equipment, "clearance", "eq", "top-secret")
        assert_respond_to rel, :to_a
        Loam::CustomFieldIndex.order(Equipment, "clearance", :asc).to_a
      end
    end
  end
  ...
  test "an open field (no readable_roles) is filterable by any member" do
    with_tenant(@warsaw, actor: @clerk) do
      assert_nothing_raised do
        Loam::CustomFieldIndex.filter(Equipment, "color", "eq", "red").to_a
      end
    end
  end
  ...
  test "a system/background context (no actor) is not gated" do
    with_tenant(@warsaw) do
      assert_nothing_raised do
        Loam::CustomFieldIndex.filter(Equipment, "clearance", "eq", "top-secret").to_a
      end
    end
  end
end
