...
# OpenLoam::CustomFieldIndex: the typed-EAV read model that makes custom-field
# filter/sort index-backed (not per-row JSON extraction).
class OpenLoamCustomFieldIndexTest < ActiveSupport::TestCase
  setup do
    @warsaw = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-cfi")
    @krakow = OpenLoam::Tenant.create!(name: "Branch Krakow", slug: "krakow-cfi")
    [ @warsaw, @krakow ].each do |t|
      with_tenant(t) do
        OpenLoam::FieldDefinition.create!(entity_type: "Equipment", name: "grade", field_type: "string")
        OpenLoam::FieldDefinition.create!(entity_type: "Equipment", name: "weight", field_type: "integer")
      end
    end
  end
  ...
  test "saving a record projects its custom fields into typed index rows" do
    ...
    with_tenant(@warsaw) do
      rows = OpenLoam::CustomFieldValue.where(indexable_type: "Equipment", indexable_id: e.id)
  ...
  test "updating a custom field re-projects (old value gone, new present)" do
    ...
    with_tenant(@warsaw) do
      assert_equal [ e.id ], OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "eq", "A").pluck(:id)
      e.set_custom_field(:grade, "B")
      e.save!
      assert_empty OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "eq", "A").pluck(:id)
      assert_equal [ e.id ], OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "eq", "B").pluck(:id)
    end
  end
  ...
  test "filter supports eq / numeric range / contains / present, index-backed" do
    ...
    with_tenant(@warsaw) do
      ...
      assert_equal [ b.id ], OpenLoam::CustomFieldIndex.filter(Equipment, "weight", "gt", 500).pluck(:id)
      assert_equal [ a.id ], OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "contains", "lph").pluck(:id)
      assert_equal 2, OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "present").count

      # The query hits the index table, NOT a json extraction on custom_fields.
      sql = OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "eq", "alpha").to_sql
      assert_match "open_loam_custom_field_values", sql
  ...
  test "order-by an indexed custom field" do
    ...
    with_tenant(@warsaw) do
      assert_equal [ a.id, b.id ], OpenLoam::CustomFieldIndex.order(Equipment, "grade", :asc).pluck(:id)
      assert_equal [ b.id, a.id ], OpenLoam::CustomFieldIndex.order(Equipment, "grade", :desc).pluck(:id)
    end
  end
  ...
  test "filter is tenant-scoped: a Warsaw custom-field value never matches from Krakow" do
    ...
    assert_equal 1, with_tenant(@krakow) { OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "eq", "shared").count }
    warsaw_id = with_tenant(@warsaw) { OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "eq", "shared").first.name }
  ...
  test "a soft-deleted record drops out of the filter (base scope), destroy removes its rows" do
    ...
    with_tenant(@warsaw) do
      e.soft_delete!
      assert_empty OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "eq", "A"), "hidden by the base scope"
      assert OpenLoam::CustomFieldValue.where(indexable_id: e.id).exists?, "rows kept on soft-delete (restore brings it back)"

      e.destroy!
      refute OpenLoam::CustomFieldValue.where(indexable_id: e.id).exists?, "hard destroy removes the rows"
    end
  end

  test "a key that is not a FieldDefinition is refused" do
    with_tenant(@warsaw) do
      assert_raises(OpenLoam::Error) { OpenLoam::CustomFieldIndex.filter(Equipment, "not_a_field", "eq", "x") }
      assert_raises(OpenLoam::Error) { OpenLoam::CustomFieldIndex.order(Equipment, "not_a_field") }
    end
  end
  ...
  test "reindex rebuilds a model's index rows from scratch" do
    ...
    with_tenant(@warsaw) do
      ...
      assert OpenLoam::CustomFieldValue.exists?(indexable_id: e.id, field_key: "grade"), "reindex rebuilt the row"
      assert_equal [ e.id ], OpenLoam::CustomFieldIndex.filter(Equipment, "grade", "eq", "A").pluck(:id)
    end
  end
end
...
class AdminCustomFieldFilterTest < ActionDispatch::IntegrationTest
  setup do
    @tenant = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-cfi-adm")
