...
# OpenLoam::BusinessRules: the when/then engine. The condition evaluator is the
...
class OpenLoamBusinessRuleConditionTest < ActiveSupport::TestCase
  setup do
    @warsaw = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-rule")
    @krakow = OpenLoam::Tenant.create!(name: "Branch Krakow", slug: "krakow-rule")
  end
  ...
  test "each comparison op and boolean nesting evaluates correctly" do
    with_tenant(@warsaw) do
      ...
      c = OpenLoam::BusinessRules::Condition
  ...
  test "an encrypted column is refused in a condition (no oracle)" do
    with_tenant(@warsaw) do
      ...
      refute OpenLoam::BusinessRules::Condition.matches?({ "field" => "tax_id", "op" => "contains", "value" => "5260" }, customer)
    end
  end
  ...
  test "conditions only ever see the record they are given, in its tenant" do
    with_tenant(@warsaw) do
      ...
      assert OpenLoam::BusinessRules::Condition.matches?({ "field" => "name", "op" => "eq", "value" => "Digger" }, equipment)
    end
  end
end

class OpenLoamBusinessRuleActionsTest < ActiveSupport::TestCase
  setup do
    @warsaw = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-act")
    @manager = User.create!(name: "M", email: "m@example.test", password: "password")
    with_tenant(@warsaw) { OpenLoam::Membership.create!(user: @manager, role: "manager") }
  end

  test "notify creates a notification for the role" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Digger", daily_rate: 10, status: "available")
      OpenLoam::BusinessRules::Actions.run({ "type" => "notify", "role" => "manager", "title" => "Hi" }, equipment)
      assert_equal 1, OpenLoam::Notification.where(title: "Hi", user_id: @manager.id).count
    end
  end
  ...
  test "emit_event publishes and validates the name" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Digger", daily_rate: 10, status: "available")
      received = []
      subscription = OpenLoam::Events.subscribe("test.rule.fired") { |_name, payload| received << payload }
      OpenLoam::BusinessRules::Actions.run({ "type" => "emit_event", "name" => "test.rule.fired", "payload" => { "id" => 1 } }, equipment)
      assert_equal 1, received.size

      assert_raises(OpenLoam::InvalidEventNameError) do
        OpenLoam::BusinessRules::Actions.run({ "type" => "emit_event", "name" => "not a valid name" }, equipment)
      end
    ensure
  ...
  test "set_field writes a whitelisted column but refuses the workflow status column" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Digger", daily_rate: 10, status: "available")
      OpenLoam::BusinessRules::Actions.run({ "type" => "set_field", "field" => "status", "value" => "rented" }, equipment)
      ...
      assert_raises(ArgumentError) do
        OpenLoam::BusinessRules::Actions.run({ "type" => "set_field", "field" => "state", "value" => "approved" }, report)
      end
...
class OpenLoamBusinessRuleEngineTest < ActiveSupport::TestCase
  setup do
    @warsaw = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-eng")
    @krakow = OpenLoam::Tenant.create!(name: "Branch Krakow", slug: "krakow-eng")
    @manager = User.create!(name: "M", email: "m@example.test", password: "password")
    with_tenant(@warsaw) { OpenLoam::Membership.create!(user: @manager, role: "manager") }
    with_tenant(@krakow) { OpenLoam::Membership.create!(user: @manager, role: "manager") }
  end

  test "the event subscriber fires a matching rule and logs the run" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Digger", daily_rate: 950, status: "available")
      OpenLoam::BusinessRule.create!(name: "pricey", entity_type: "Equipment", trigger: "rental.equipment.pinged",
                                 condition: { "field" => "daily_rate", "op" => "gt", "value" => 500 },
                                 actions: [ { "type" => "notify", "role" => "manager", "title" => "Pricey" } ], active: true)

      OpenLoam::Events.publish("rental.equipment.pinged", id: equipment.id)

      assert_equal 1, OpenLoam::Notification.where(title: "Pricey").count
      run = OpenLoam::BusinessRuleRun.last
      assert run.matched
  ...
  test "rules fire in priority order" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Digger", daily_rate: 950, status: "available")
      OpenLoam::BusinessRule.create!(name: "low", entity_type: "Equipment", trigger: "rental.equipment.pinged",
                                 condition: {}, actions: [ { "type" => "notify", "role" => "manager", "title" => "B-low" } ], priority: 1, active: true)
      OpenLoam::BusinessRule.create!(name: "high", entity_type: "Equipment", trigger: "rental.equipment.pinged",
                                 condition: {}, actions: [ { "type" => "notify", "role" => "manager", "title" => "A-high" } ], priority: 10, active: true)

      OpenLoam::Events.publish("rental.equipment.pinged", id: equipment.id)

      assert_equal [ "A-high", "B-low" ], OpenLoam::Notification.order(:id).pluck(:title), "higher priority ran first"
    end
  end
  ...
  test "a raising rule is isolated: it logs an error and does not break sibling rules or dispatch" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Digger", daily_rate: 10, status: "available")
      OpenLoam::BusinessRule.create!(name: "boom", entity_type: "Equipment", trigger: "rental.equipment.pinged",
                                 condition: {}, actions: [ { "type" => "set_field", "field" => "no_such_field", "value" => 1 } ], priority: 10, active: true)
      OpenLoam::BusinessRule.create!(name: "ok", entity_type: "Equipment", trigger: "rental.equipment.pinged",
                                 condition: {}, actions: [ { "type" => "notify", "role" => "manager", "title" => "Still ran" } ], priority: 1, active: true)

      assert_nothing_raised { OpenLoam::Events.publish("rental.equipment.pinged", id: equipment.id) }

      assert_equal 1, OpenLoam::Notification.where(title: "Still ran").count, "the sibling rule still ran"
      assert OpenLoam::BusinessRuleRun.where(matched: false).where.not(error: nil).exists?, "the failure was logged"
    end
  end

  test "a Warsaw rule never fires on a Krakow event" do
    with_tenant(@warsaw) do
      OpenLoam::BusinessRule.create!(name: "w-rule", entity_type: "Equipment", trigger: "rental.equipment.pinged",
    ...
    assert_equal 0, with_tenant(@warsaw) { OpenLoam::Notification.where(title: "W").count }, "the Warsaw rule stayed in Warsaw"
  end

  test "a self-triggering rule terminates (depth-bounded), it does not hang" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Digger", daily_rate: 10, status: "available")
      OpenLoam::BusinessRule.create!(name: "loop", entity_type: "Equipment", trigger: "rental.loop.tick",
                                 condition: {}, actions: [ { "type" => "emit_event", "name" => "rental.loop.tick", "payload" => { "id" => equipment.id } } ], active: true)

      assert_nothing_raised { OpenLoam::Events.publish("rental.loop.tick", id: equipment.id) }
      assert_operator OpenLoam::BusinessRuleRun.where(event_name: "rental.loop.tick").count, :<=, OpenLoam::BusinessRules::MAX_DEPTH
    end
  end

  test "a block_transition rule vetoes a workflow transition" do
    with_tenant(@warsaw, actor: @manager) do
      OpenLoam::BusinessRule.create!(name: "no-critical", entity_type: "DamageReport", trigger: "rental.damage_report.submit",
      ...
      assert_raises(OpenLoam::TransitionVetoedError) { critical.submit! }
...
class OpenLoamBusinessRuleSecurityTest < ActiveSupport::TestCase
  setup do
    @warsaw = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-br-sec")
    @krakow = OpenLoam::Tenant.create!(name: "Branch Krakow", slug: "krakow-br-sec")
  end

  test "a rule may only target a tenant-scoped model, refused at save time" do
    with_tenant(@warsaw) do
      assert OpenLoam::BusinessRule.new(name: "ok", trigger: "rental.x", entity_type: "Equipment").valid?
      assert OpenLoam::BusinessRule.new(name: "event-only", trigger: "rental.x", entity_type: nil).valid?, "an event-only rule needs no entity"

      user_rule = OpenLoam::BusinessRule.new(name: "pwn", trigger: "rental.x", entity_type: "User")
      refute user_rule.valid?, "a global model like User is refused"
      assert user_rule.errors[:entity_type].any?

      refute OpenLoam::BusinessRule.new(name: "j1", trigger: "rental.x", entity_type: "Kernel").valid?, "a non-model class is refused"
      refute OpenLoam::BusinessRule.new(name: "j2", trigger: "rental.x", entity_type: "NotARealModel").valid?, "an unresolvable name is refused"
    end
  end
  ...
  test "a poisoned rule targeting a global model resolves NO subject and cannot mutate a User" do
    ...
    rule = with_tenant(@warsaw) do
      r = OpenLoam::BusinessRule.create!(name: "takeover", trigger: "rental.pwn.attempt", entity_type: "Equipment",
    ...
    with_tenant(@warsaw) do
      assert_nil OpenLoam::BusinessRules.send(:subject_for, rule, { id: victim.id }), "subject_for refuses a non-TenantRecord"
      assert_nothing_raised { OpenLoam::Events.publish("rental.pwn.attempt", id: victim.id) }
    end
  ...
  test "the emit_event -> set_field chain cannot reach a User across the depth guard" do
    ...
    with_tenant(@warsaw) do
      ...
      OpenLoam::BusinessRule.create!(name: "A", trigger: "rental.pwn.start", entity_type: "Equipment", condition: {},
      ...
      b = OpenLoam::BusinessRule.create!(name: "B", trigger: "rental.pwn.exec", entity_type: "Equipment", condition: {},
      ...
      assert_nothing_raised { OpenLoam::Events.publish("rental.pwn.start", id: equipment.id) }
    end
  ...
  test "set_field refuses a credential column even when the record really has one" do
    with_tenant(@warsaw) do
      ...
      assert_raises(ArgumentError) do
        OpenLoam::BusinessRules::Actions.run({ "type" => "set_field", "field" => "email", "value" => "attacker@evil.test" }, customer)
      end
      ...
      %w[password_digest encrypted_password PASSWORD some_digest].each do |field|
        assert_raises(ArgumentError, "#{field} must be refused") do
          OpenLoam::BusinessRules::Actions.run({ "type" => "set_field", "field" => field, "value" => "x" }, customer)
        end
      end
    end
  end
