...
class OpenLoamUndoTest < ActiveSupport::TestCase
  setup do
    @tenant = OpenLoam::Tenant.create!(name: "Branch", slug: "warsaw-undo")
    @other = OpenLoam::Tenant.create!(name: "Other", slug: "other-undo")
    @anna = User.create!(name: "Anna", email: "anna-undo@example.test", password: "password")
  end

  def latest_update_audit(record)
    OpenLoam::AuditRecord.where(auditable_type: record.class.name, auditable_id: record.id, action: %w[update undo]).order(:id).last
  end
  ...
  test "undoing an update reverts the field and records the undo as its own audit" do
    with_tenant(@tenant, actor: @anna) do
      ...
      OpenLoam::Undo.undo(latest_update_audit(eq))
  ...
  test "redo = undo the undo, re-applying the change" do
    with_tenant(@tenant, actor: @anna) do
      ...
      OpenLoam::Undo.undo(latest_update_audit(eq))     # back to Digger, writes an "undo" audit
      assert_equal "Digger", eq.reload.name

      OpenLoam::Undo.undo(latest_update_audit(eq))     # undo the undo = redo
  ...
  test "an older change can't be undone while a newer one exists (no silent clobber)" do
    with_tenant(@tenant, actor: @anna) do
      ...
      error = assert_raises(OpenLoam::Undo::NotUndoableError) { OpenLoam::Undo.undo(first_update) }
  ...
  test "an audit that only touched encrypted fields has nothing to revert" do
    with_tenant(@tenant, actor: @anna) do
      ...
      assert_raises(OpenLoam::Undo::NotUndoableError) { OpenLoam::Undo.undo(audit) }
    end
  end
  ...
  test "the workflow column is never reverted by a direct write" do
    with_tenant(@tenant, actor: @anna) do
      ...
      assert_raises(OpenLoam::Undo::NotUndoableError) { OpenLoam::Undo.undo(transition_audit) }
  ...
  test "undoing a create soft-deletes; undoing that (restore) brings it back" do
    with_tenant(@tenant, actor: @anna) do
      eq = Equipment.create!(name: "Digger", daily_rate: 100, status: "available")
      create_audit = OpenLoam::AuditRecord.where(auditable_type: "Equipment", auditable_id: eq.id, action: "create").first

      OpenLoam::Undo.undo(create_audit)
      assert eq.reload.deleted?, "undo of a create soft-deletes"

      soft_delete_audit = OpenLoam::AuditRecord.where(auditable_type: "Equipment", auditable_id: eq.id, action: "soft_delete").last
      OpenLoam::Undo.undo(soft_delete_audit)
  ...
  test "a policy withholds fields the role may not write" do
    with_tenant(@tenant, actor: @anna) do
      ...
      assert_raises(OpenLoam::Undo::NotUndoableError) { OpenLoam::Undo.undo(audit, policy: policy) }
  ...
  test "an audit from another tenant is refused" do
    ...
    with_tenant(@tenant, actor: @anna) do
      assert_raises(OpenLoam::Undo::NotUndoableError) { OpenLoam::Undo.undo(other_audit) }
    end
  end
end

class OpenLoamHistoryScreenTest < ActionDispatch::IntegrationTest
  setup do
    @tenant = OpenLoam::Tenant.create!(name: "Branch", slug: "warsaw-hist")
  ...
  test "the history screen lists changes and the undo button reverts one" do
    ...
    audit = with_tenant(@tenant) { OpenLoam::AuditRecord.where(auditable_type: "Equipment", auditable_id: @eq.id, action: "update").order(:id).last }
  ...
  test "history for a record in another tenant is not reachable" do
    other = OpenLoam::Tenant.create!(name: "Other", slug: "other-hist")
