...
# which check failed. Exercised through Loam::InboundWebhooks.ingest (no HTTP).
class LoamInboundWebhooksTest < ActiveSupport::TestCase
  SECRET = "shhh-secret-key".freeze

  setup do
    @warsaw = Loam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-inbound")
  ...
  def ingest(body, headers, token: @source.token)
    Loam::InboundWebhooks.ingest(token: token, raw_body: body, headers: headers)
  ensure
    Loam::Current.reset
  end

  def deliveries
    with_tenant(@warsaw) { Loam::InboundWebhookDelivery.count }
  end

  test "a valid signature is accepted, recorded, and published on the bus" do
    body = %({"id":7})
    received = []
    sub = Loam::Events.subscribe("demo.inbound.received") { |_n, payload| received << payload }
    result = ingest(body, { "X-Loam-Signature" => sign(body) })
    ActiveSupport::Notifications.unsubscribe(sub)

    assert_equal 202, result.status
    with_tenant(@warsaw) do
      delivery = Loam::InboundWebhookDelivery.sole
  ...
  test "a bad signature is rejected 401 and records nothing" do
    body = %({"id":7})
    assert_equal 401, ingest(body, { "X-Loam-Signature" => sign(body, "wrong-key") }).status
    assert_equal 0, deliveries
  end

  test "a wrong-length signature is 401, never a 500" do
    body = %({"id":7})
    assert_equal 401, ingest(body, { "X-Loam-Signature" => "sha256=short" }).status
    assert_equal 401, ingest(body, { "X-Loam-Signature" => "" }).status
    assert_equal 401, ingest(body, {}).status
  end

  test "an unknown or inactive source is 404 (before any signature work)" do
    body = %({"id":7})
    assert_equal 404, ingest(body, { "X-Loam-Signature" => sign(body) }, token: "nope").status

    with_tenant(@warsaw) { @source.update!(active: false) }
    assert_equal 404, ingest(body, { "X-Loam-Signature" => sign(body) }).status
  end

  test "a body over the size cap is 413 before anything else" do
    big = "x" * (Loam::InboundWebhooks::MAX_BYTES + 1)
    assert_equal 413, ingest(big, { "X-Loam-Signature" => sign(big) }).status
    assert_equal 0, deliveries
  end
  ...
  test "a replay (same external_id) is idempotent — 200, one row, published once" do
    body = %({"id":7})
    publishes = 0
    sub = Loam::Events.subscribe("demo.inbound.received") { |_n, _p| publishes += 1 }

    first = ingest(body, { "X-Loam-Signature" => sign(body) })   # body-hash external_id
    second = ingest(body, { "X-Loam-Signature" => sign(body) })
    ActiveSupport::Notifications.unsubscribe(sub)
  ...
  test "the delivery-id header dedupes even when bodies differ" do
    ...
    a = ingest(%({"n":1}), { "X-Loam-Signature" => sign(%({"n":1})), "X-Delivery-Id" => "abc" })
    b = ingest(%({"n":2}), { "X-Loam-Signature" => sign(%({"n":2})), "X-Delivery-Id" => "abc" })
  ...
  test "a configured timestamp header enforces a freshness window" do
    ...
    assert_equal 401, ingest(body, { "X-Loam-Signature" => sign(body), "X-Timestamp" => stale }).status

    fresh = Time.current.to_i.to_s
    assert_equal 202, ingest(body, { "X-Loam-Signature" => sign(body), "X-Timestamp" => fresh }).status
  end

  test "a publish failure rolls the delivery row back (retry-safe)" do
    body = %({"id":7})
    sub = Loam::Events.subscribe("demo.inbound.received") { |_n, _p| raise "subscriber blew up" }

    assert_raises(RuntimeError) { ingest(body, { "X-Loam-Signature" => sign(body) }) }
    ActiveSupport::Notifications.unsubscribe(sub)
