...
# OpenLoam::Dictionaries: per-tenant managed lookup lists, usable as a custom-field
# type. Tenant-scoped, ordered, cached per request.
class OpenLoamDictionaryTest < ActiveSupport::TestCase
  setup do
    @warsaw = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-dict")
    @krakow = OpenLoam::Tenant.create!(name: "Branch Krakow", slug: "krakow-dict")

    with_tenant(@warsaw) do
      dict = OpenLoam::Dictionary.create!(key: "severity", name: "Severity")
  ...
  test "get / entries / default / label_for resolve for the current tenant" do
    with_tenant(@warsaw) do
      assert_equal "Severity", OpenLoam::Dictionaries.get("severity").name
      assert_equal %w[minor major critical], OpenLoam::Dictionaries.entries("severity").map(&:value), "ordered by position, inactive excluded"
      assert_equal "critical", OpenLoam::Dictionaries.default("severity").value
      assert_equal "Critical", OpenLoam::Dictionaries.label_for("severity", "critical")
    end
  end

  test "label_for returns the raw value for an unknown value or key" do
    with_tenant(@warsaw) do
      assert_equal "zzz", OpenLoam::Dictionaries.label_for("severity", "zzz")
      assert_equal "anything", OpenLoam::Dictionaries.label_for("no_such_dict", "anything")
    end
  end

  test "a dictionary is invisible from another tenant" do
    with_tenant(@krakow) do
      assert_nil OpenLoam::Dictionaries.get("severity")
      assert_empty OpenLoam::Dictionaries.entries("severity")
    end
  end

  test "an entry value is unique within its dictionary" do
    with_tenant(@warsaw) do
      dict = OpenLoam::Dictionaries.get("severity")
  ...
  test "a dictionary-typed custom field round-trips a value and resolves its label" do
    with_tenant(@warsaw) do
      OpenLoam::FieldDefinition.create!(entity_type: "DamageReport", name: "sev", field_type: "dictionary", dictionary_key: "severity")
  ...
  test "a dictionary field definition requires an existing dictionary" do
    with_tenant(@warsaw) do
      missing = OpenLoam::FieldDefinition.new(entity_type: "DamageReport", name: "s2", field_type: "dictionary", dictionary_key: "nope")
      refute missing.valid?
      assert missing.errors[:dictionary_key].any?

      blank = OpenLoam::FieldDefinition.new(entity_type: "DamageReport", name: "s3", field_type: "dictionary")
...
class OpenLoamDictionaryAdminTest < ActionDispatch::IntegrationTest
  setup do
    @tenant = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-dict-admin")
  ...
  test "a manager creates a dictionary and adds an entry" do
    ...
    dictionary = with_tenant(@tenant) { OpenLoam::Dictionary.find_by(key: "vehicle_category") }
    ...
    assert_equal "Van", with_tenant(@tenant) { OpenLoam::Dictionaries.label_for("vehicle_category", "van") }
  end
  ...
  test "a dictionary-typed custom field renders a select of its entries on the entity form" do
    report = with_tenant(@tenant) do
      ...
      OpenLoam::FieldDefinition.create!(entity_type: "DamageReport", name: "severity", field_type: "dictionary", dictionary_key: "severity")
