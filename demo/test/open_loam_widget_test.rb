...
# OpenLoam::Widgets (registry) + OpenLoam::Dashboard (per-tenant, role-visible layout).
class OpenLoamWidgetTest < ActiveSupport::TestCase
  setup do
    @warsaw = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-widget")
    @krakow = OpenLoam::Tenant.create!(name: "Branch Krakow", slug: "krakow-widget")
    @actor = User.create!(name: "A", email: "a@example.test", password: "password")
    OpenLoam::Widgets.reset!
  end

  # Restore the boot-time built-in widgets for the rest of the suite.
  teardown { OpenLoam::Widgets.reset!; OpenLoam::Widgets.register_builtins! }

  test "the dashboard renders the tenant's configured widgets in order" do
    OpenLoam::Widgets.register(key: "a", title: "A") { |_| { kind: "count", value: 1 } }
    OpenLoam::Widgets.register(key: "b", title: "B") { |_| { kind: "count", value: 2 } }

    with_tenant(@warsaw, actor: @actor) do
      OpenLoam::DashboardWidget.create!(widget_key: "b", position: 1)
      OpenLoam::DashboardWidget.create!(widget_key: "a", position: 2)

      tiles = OpenLoam::Dashboard.for(actor: @actor, role: :manager)
  ...
  test "the default set (every registered widget) renders when unconfigured" do
    ...
    with_tenant(@krakow, actor: @actor) do
      assert_equal %w[a b], OpenLoam::Dashboard.for(actor: @actor, role: :manager).map { |t| t[:key] }
    end
  end

  test "a role-restricted widget is hidden AND its data is never computed for that role" do
    computed = false
    OpenLoam::Widgets.register(key: "secret", title: "Secret", roles: %w[manager]) do |_|
      computed = true
      { kind: "count", value: 42 }
    end

    with_tenant(@warsaw, actor: @actor) do
      tiles = OpenLoam::Dashboard.for(actor: @actor, role: :employee)
      ...
      OpenLoam::Dashboard.for(actor: @actor, role: :manager)
      assert computed, "a manager does compute it"
    end
  end

  test "a raising widget is isolated into an error tile; the others still render" do
    OpenLoam::Widgets.register(key: "ok", title: "OK") { |_| { kind: "count", value: 1 } }
    OpenLoam::Widgets.register(key: "boom", title: "Boom") { |_| raise "kaboom" }
  ...
  test "a widget's data is current-tenant only" do
    OpenLoam::Widgets.register(key: "equipment_count", title: "Equipment") { |_| { kind: "count", value: Equipment.count } }
    ...
    warsaw_value = with_tenant(@warsaw, actor: @actor) { OpenLoam::Widgets.resolve("equipment_count", actor: @actor, role: :manager)[:data][:value] }
    krakow_value = with_tenant(@krakow, actor: @actor) { OpenLoam::Widgets.resolve("equipment_count", actor: @actor, role: :manager)[:data][:value] }
  ...
  test "config is per tenant: Warsaw's layout doesn't apply to Krakow" do
    OpenLoam::Widgets.register(key: "a", title: "A") { |_| { kind: "count", value: 1 } }
    OpenLoam::Widgets.register(key: "b", title: "B") { |_| { kind: "count", value: 2 } }
    with_tenant(@warsaw, actor: @actor) { OpenLoam::DashboardWidget.create!(widget_key: "b", position: 1) }

    warsaw = with_tenant(@warsaw, actor: @actor) { OpenLoam::Dashboard.for(actor: @actor, role: :manager).map { |t| t[:key] } }
    krakow = with_tenant(@krakow, actor: @actor) { OpenLoam::Dashboard.for(actor: @actor, role: :manager).map { |t| t[:key] } }
  ...
  test "the built-in widgets register and resolve" do
    OpenLoam::Widgets.register_builtins!
    with_tenant(@warsaw, actor: @actor) do
      OpenLoam::Membership.create!(user: @actor, role: "manager")
      tiles = OpenLoam::Dashboard.for(actor: @actor, role: :manager)
...
class AdminDashboardWidgetsTest < ActionDispatch::IntegrationTest
  setup do
    @tenant = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-dw-admin")
  ...
  test "a manager configures the dashboard; an employee cannot" do
    ...
    assert_equal 5, with_tenant(@tenant) { OpenLoam::DashboardWidget.find_by(widget_key: "audit_recent").position }
