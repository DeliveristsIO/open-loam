...
class AdminCommentsTest < ActionDispatch::IntegrationTest
  setup do
    @tenant = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-admin-comments")
  ...
  test "a type that is not a commentable OpenLoam entity is refused" do
    # A real class, but not a OpenLoam entity — the check is what the class IS,
    # not whether the name resolves.
    post admin_comments_path, params: { commentable_type: "User", commentable_id: @anna.id, body: "nope" }
    assert_response :forbidden

    post admin_comments_path, params: { commentable_type: "OpenLoam::Tenant", commentable_id: @tenant.id, body: "nope" }
    assert_response :forbidden
    ...
    with_tenant(@tenant) { assert_equal 0, OpenLoam::Comment.count }
  end

  test "a record in another tenant is not found rather than commentable" do
    other_tenant = OpenLoam::Tenant.create!(name: "Branch Krakow", slug: "krakow-admin-comments")
    ...
    with_tenant(other_tenant) { assert_equal 0, OpenLoam::Comment.count }
  end
