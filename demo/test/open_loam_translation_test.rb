...
# OpenLoam::Translatable: per-record, per-locale content overlay over the base column.
class OpenLoamTranslationTest < ActiveSupport::TestCase
  setup do
    @warsaw = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-trans")
    @krakow = OpenLoam::Tenant.create!(name: "Branch Krakow", slug: "krakow-trans")
    OpenLoam::Current.locale = nil
  end

  teardown { OpenLoam::Current.locale = nil }
  ...
  test "translations are request-scoped: switching OpenLoam::Current.locale changes reads" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Excavator", daily_rate: 1, status: "available")
      equipment.set_translation(:name, "de", "Bagger")
      equipment.set_translation(:name, "pl", "Koparka")

      OpenLoam::Current.locale = "pl"
  ...
  test "a translation is invisible from another tenant" do
    ...
    with_tenant(@krakow) { assert_equal 0, OpenLoam::Translation.count, "Warsaw's translation is not visible in Krakow" }
    assert_equal 1, with_tenant(@warsaw) { OpenLoam::Translation.where(translatable: equipment).count }
  end
  ...
  test "a translation value is unique per (record, locale, field)" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Excavator", daily_rate: 1, status: "available")
      equipment.set_translation(:name, "de", "Bagger")
      dup = OpenLoam::Translation.new(translatable: equipment, locale: "de", field: "name", value: "X")
      refute dup.valid?
  ...
  test "set_translation refuses a non-translatable field" do
    with_tenant(@warsaw) do
      equipment = Equipment.create!(name: "Excavator", daily_rate: 1, status: "available")
      assert_raises(OpenLoam::Error) { equipment.set_translation(:status, "de", "x") }
    end
  end
end
...
class AdminTranslationsTest < ActionDispatch::IntegrationTest
  setup do
    @tenant = OpenLoam::Tenant.create!(name: "Branch Warsaw", slug: "warsaw-trans-admin")
